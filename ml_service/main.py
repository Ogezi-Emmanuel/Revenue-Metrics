...
def get_pipeline_intelligence():
    try:
        if not os.path.exists("synthetic_pipeline_payload.csv"):
            raise FileNotFoundError("Master CSV database not found.")
        ...
        df['stage_lower'] = df['stage'].astype(str).str.lower().str.replace(' ', '')
        open_pipe = df[~df['stage_lower'].isin(['closedwon', 'closedlost'])].copy()
        ...
        elevated_pipe = open_pipe[(open_pipe['flight_risk_pct'] >= 35.0) & (open_pipe['flight_risk_pct'] < 65.0)]
        ...
        sorted_pipe = open_pipe.sort_values(by=['flight_risk_pct', 'amount'], ascending=[False, False])
        ...
        payload = {
            ...
            "opportunities": sorted_pipe[[
                'deal_name', 'company_name', 'stage', 'amount', 
                'flight_risk_pct', 'est_days_left'
        ...
        raise HTTPException(status_code=500, detail=f"Data Presentation failure: {str(e)}")